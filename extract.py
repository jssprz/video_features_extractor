...
def extract_features(
    ...
    if dataset_name == "MSVD":
        ...
        videos = [
            os.path.join(config.videos_dir, v)
            for v in sorted(os.listdir(config.videos_dir), key=lambda x: int(x[5:-4]))
        ]
        ...
        videos = [
            os.path.join(config.videos_dir, v)
            for v in sorted(os.listdir(config.videos_dir), key=lambda x: int(x[6 : x.index(".")]))
        ]
    elif dataset_name in ["TRECVID-2020-Test", "TRECVID-2021-Test"]:
        videos = [
            os.path.join(config.videos_dir, v)
            for v in sorted(os.listdir(config.videos_dir), key=lambda x: int(x[: x.index(".")]))
        ]
    elif dataset_name == "TGIF":
        videos = [
            os.path.join(config.videos_dir, v)
            for v in sorted(os.listdir(config.videos_dir), key=lambda x: int(x[:-4]))
        ]
    ...
    print("initializing the datasets in the h5 file...")
    dataset_models = {}
    if dataset_name in list(h5.keys()):
        dataset = h5[dataset_name]
        for extractor_name, feature_size in zip(extractor_names, feature_sizes):
            if extractor_name in list(dataset.keys()):
                dataset_model = dataset[extractor_name]
            elif extractor_name.split("_")[-1] == "features":
                dataset_model = dataset.create_dataset(
                    extractor_name, (len(videos), config.max_frames, feature_size), dtype="float32"
                )
                ...
                dataset_model = dataset.create_dataset(
                    extractor_name, (len(videos), config.max_frames), dtype="float32"
                )
            dataset_models[extractor_name] = dataset_model
    ...
    print("Moving the extractor models to device...")
    ...
    for i in range(len(videos)):
        ...
        sample_type = "dynamic"  # "fixed" if extractor_name == "eco_globals" else "dynamic"
        max_frames = config.max_frames  # 16 if extractor_name == "eco_globals" else config.max_frames
        frame_list, frame_ts_list, chunk_list, frame_count, fragments_mask = sample_frames(
            sample_type,
            video_path,
            max_frames,
        ...
        if frame_count < config.chunk_size:
            print(
                f"The {i} video: {video_path} was discarded because it has less than {config.chunk_size} correct frames."
            )
            forget_idxs.append(i)
            if dataset_name in ["TRECVID-2020", "VATEX", "TGIF", "ActivityNet-Dense"]:
                i += 1
        ...
        if dataset_name == "MSVD":
            ...
            map_name_timestamp_idx = "{} \t {} \t {} \t {} \t {}\n".format(
                video_path.split("/")[-1].rsplit(".", 1)[0], fragment[0], fragment[1], fragment_idx, i
            )
        ...
        if i % logging == 0:
            if fragment is None:
                print(
                    "%d\t%s\ttotal frames: %d\tsampled frames/chunks: %d"
                    % (i, video_path.split("/")[-1], frame_count, len(frame_list))
                )
            else:
                print("%d\t%s\t[%f,%f]\t%d" % (i, video_path.split("/")[-1], fragment[0], fragment[1], frame_count))

        for extractor_name, extractor, transformer, feature_size in zip(
            extractor_names, extractors, transformers, feature_sizes
        ):
            # extract feature from sampled frames
            if extractor_name == "events_mask":
                assert (
                    fragments_mask is not None
                ), "to compute the mask you must pass the list of fragments to the sample_frames method"
                ...
                b, n, features = config.cnn_batch_size, len(frame_list), []
                for j in range(0,n,b):
                    batch = torch.stack([transformer(x) for x in frame_list[j:min(j + b, n)]]).to(device)
                    
                    # extract features for current batch
                    features.append(extractor(batch))
                ...
                b, n, features = config.c3d_batch_size, len(chunk_list), []
                ...
                for j in range(0, n, b):
                    ...
                    clips_batch = (
                        torch.stack(
                            [
                                transformer(torch.stack([totensor(im) for im in c]))
                                for c in chunk_list[j : min(j + b, n)]
                            ]
                        )
                        .transpose(1, 2)
                        .to(device)
                ...
                frames = np.array(
                    [
                        preprocess_frame(x, scale_size=scale_size, crop_size=crop_size, mean=input_mean, std=input_std)
                        for x in frame_list
                    ]
                )
                    ...
                    chunk_frames = (
                        torch.cat(
                            [
                                torch.from_numpy(
                                    preprocess_frame(
                                        x, scale_size=scale_size, crop_size=crop_size, mean=input_mean, std=input_std
                                    )
                                )
                                for x in clip
                            ],
                ...
                frames = (
                    torch.cat(
                        [
                            torch.from_numpy(
                                preprocess_frame(x, crop_size=224, scale_size=256, mean=[104, 117, 128], std=[1])
                            )
                            for x in frame_list
                        ],
                        dim=2,
            ...
            if extractor_name.split("_")[-1] == "features":
                dataset_models[extractor_name][i] = np.zeros((config.max_frames, feature_size), dtype="float32")
                dataset_models[extractor_name][i, : features.size(0), :] = features.data.cpu().numpy()
                ...
                dataset_models[extractor_name][i] = features.data.cpu().numpy()
...
def main_separated(args, config, h5_path):
    for feats_name in args.features:
        ...
        if feats_name == "cnn_features":
            ...
            model = CNN2D(config.cnn_model, input_size=224, use_pretrained=cnn_use_torch_weights, use_my_resnet=False)
            if not cnn_use_torch_weights:
                model.load_pretrained(config.cnn_pretrained_path)
            transformer = transforms.Compose(
                [
                    transforms.Scale(model.scale_size),
        ...
        if feats_name in ["cnn_globals", "cnn_sem_globals"]:
            ...
            model = CNN2D(
                ...
                use_pretrained=True,
                use_my_resnet=False,
                get_probs=feats_name == "cnn_sem_globals",
            )
            transformer = transforms.Compose(
                [
        ...
        if feats_name in ["eco_features", "eco_globals"]:
            ...
            model, crop_size, scale_size, input_mean, input_std = ECO(
                num_class=400,
                num_segments=16,
        ...
        if feats_name in ["eco_sem_features", "eco_sem_globals"]:
            ...
            model, crop_size, scale_size, input_mean, input_std = ECO(
                num_class=400,
                num_segments=config.frame_sample_rate,
                pretrained_parts="finetune",  #'2D', '3D',
                modality="RGB",
                arch="ECOfull",  # 'ECOfull' 'ECO' 'C3DRes18'
                consensus_type="identity",  #'avg',
        ...
        if feats_name in ["tsm_features", "tsm_globals"]:
            ...
            model, crop_size, scale_size, input_mean, input_std = TSM(
                num_class=174,
                num_segments=config.frame_sample_rate,
                modality="RGB",
...
def main_join(args, config, h5_path):
    # initialize extractors
    models, transformers, feature_sizes = [], [], []
    for feats_name in args.features:
        if feats_name == "cnn_features":
            print(f"Initializing CNN for {config.dataset_name} dataset")
            cnn_use_torch_weights = config.cnn_pretrained_path == None
            model = CNN2D(config.cnn_model, input_size=224, use_pretrained=cnn_use_torch_weights, use_my_resnet=False)
            if not cnn_use_torch_weights:
                model.load_pretrained(config.cnn_pretrained_path)
        ...
        elif feats_name in ["c3d_features", "c3d_globals"]:
            print(f"Initializing C3D for {config.dataset_name} dataset")
            model = C3D()
            model.load_pretrained(config.c3d_pretrained_path)
            transformer = transforms.Compose(
                [
        ...
        feature_sizes.append(model.feature_size)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--split", type=str, default="train", help="Set the name of the split (default is train)."
    )
    parser.add_argument(
        "-log", "--logging", type=int, default=10, help="Set the number of steps between logging (default is 10)."
    )
    parser.add_argument(
        "-f", "--features", nargs="+", help="<Required> Set the names of features to be extracted", required=True
    )
    parser.add_argument(
        "-m",
        "--mode",
        type=str,
        default="separated",
        help='[separated, join] Use "separated" for extracting features one by one, and "join" for extracting features at the same time',
    )
    ...
    file_name = "{}_feats_linspace{}_{}-{}.h5".format(
        args.split.lower(), config.frame_sample_rate, config.max_frames, "-".join(args.features)
    )
    ...
    if args.mode == "separated":
        main_separated(args, config, h5_path)
    elif args.mode == "join":
        main_join(args, config, h5_path)
    else:
